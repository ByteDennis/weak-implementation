...
if __name__ == "__main__":
    import sys, shlex  # noqa: E401
    from skweak.utils import DATA_HOME

    data_name = 'mushroom'
    data_dir  =  DATA_HOME / "OPENML"
...
def get_rules(tree):
    ...
    features = [i if i != _tree.TREE_UNDEFINED else "undefined!" for i in tree_.feature]
...
def main(args):
    ...
    if np.any(categorical_indicator):
        X.loc[:, categorical_indicator] = X.loc[:, categorical_indicator].astype('object')
        X.loc[:, categorical_indicator] = X.loc[:, categorical_indicator].apply(
    ...
    for l, idx in label2idx.items():
        subset_idx.extend(
            np.random.choice(idx, size=min(subset_label_size, len(idx)), replace=False)
        )
    ...
    for single_tree in trees:
        ...
        cur_rules_text = get_rules_text(
            single_tree, feature_names=attribute_names, class_names=labels
        )
    ...
    train_cut, valid_cut = int(N * 0.8), int(N * 0.9)
    train_idx, valid_idx, test_idx = (
        indices[:train_cut],
        indices[train_cut:valid_cut],
        indices[valid_cut:],
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_dir", required=False, default="../", help="folder to save dataset"
    )
    parser.add_argument(
        "--class_encode",
        required=False,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="percent of data used for generating rules",
    )
