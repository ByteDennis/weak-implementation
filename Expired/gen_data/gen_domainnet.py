...
method: use non-target domain data to train weak labelers. data is obtained from here: https://ai.bu.edu/M3SDA/

source: Mazzetto, Alessio, et al. "Adversarial multi class learning under weak supervision with performance guarantees." International conference on machine learning. PMLR, 2021.
"""

if __name__ == '__main__':
    import sys, shlex
    from skweak.utils import DATA_HOME
...
class DomainNet:
    ...
    def sample_data_from_domain(self, domain: str, classes: list, n_split: int = 1):
        ...
        datasets = [
            DomainNetDataset(self.data_root, domain, class2data)
            for c2d in labeler_class2data
        ]
        return datasets
...
def train_labelers(datasets, model_name, pretrained, n_epochs, lr, batch_size, device):
    ...
    labeler, model_classes = train(
        datasets, datasets, model_name, pretrained, n_epochs, lr, batch_size, device
    )
    labelers = [labeler]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n_epochs", default=100, type=int, help="number of training epochs"
    )
    ...
    target_dataset = doaminnet_data.sample_data_from_domain(
        args.target_domain, sampled_classes, 1
    )[0]
    labeler_domains = []
    labeler_classes = []
    labeler_predictions = []
    ...
    for d in DOMAINS:
        if d != args.target_domain:
            sampled_datasets = doaminnet_data.sample_data_from_domain(
                d, sampled_classes, args.n_labeler_per_domain
            )
            labelers, model_classes_l = train_labelers(
                sampled_datasets,
                args.model_name,
                args.pretrained,
                args.n_epochs,
            ...
            for labeler, model_classes in zip(labelers, model_classes_l):
                predictions, probas = apply(
                    labeler, target_dataset, model_classes, device, batch_size=512
                )
